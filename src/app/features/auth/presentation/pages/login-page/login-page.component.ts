...
export class LoginPageComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (returnUrl) {
      this.errorMessage = 'Inicia sesión para continuar con tu solicitud o matrícula.';
    }
  }
  ...
  private redirectUser(role: string): void {
    const returnUrl =
      sessionStorage.getItem('enrollmentReturnUrl') || this.getReturnUrlFromQuery();

    if (returnUrl) {
      sessionStorage.removeItem('enrollmentReturnUrl');
      const decoded = decodeURIComponent(returnUrl);

      // Si es una URL externa (cross-origin como Lumina.Web de matrícula)
      if (decoded.startsWith('http://') || decoded.startsWith('https://')) {
        window.location.href = decoded;
        return;
      }

      // Si es una ruta interna relativa
