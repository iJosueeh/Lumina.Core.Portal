...
export class ProfileViewComponent implements OnInit {
  ...
  progressPercentage = computed(() => {
    const p = this.profile();
    return p ? Math.round((p.creditosAprobados / p.totalCreditos) * 100) : 0;
  });

  socialLinksArray = computed(() => {
    const p = this.profile();
    if (!p?.redesSociales) return [];

    return Object.entries(p.redesSociales)
      .filter(([_, url]) => url)
      .map(([platform, url]) => ({
  ...
  private getPlatformIcon(platform: string): string {
    const icons: Record<string, string> = {
      ...
      portfolio: 'M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5',
      facebook:
